...
// !!!
//UNCOMMENT THIS IF WANT TO RUN std::bind
// !!!
//#include <functional>
//namespace n = std;
//using std::bind;
...
static int take_first( int first, int )
{
    return first;
}

static int take_second( int, int second )
{
    return second;
}
...
static void test_passing_by_rvalue_ref()
{
    ...
    test_struct arg1;
    bind( func_arb_arg < test_struct && > , _1 )( std::move( arg1 ) );
    assert( !arg1.was_moved_ );

    test_struct arg2;
    func_arb_arg< test_struct&&>( std::move( arg2 ) );
    assert( !arg2.was_moved_ );
    ( void )src;
}
...
static std::string identity_str( const std::string& s )
{
    return s;
}
static std::string string_cat( const std::string& s1, const std::string& s2 )
{
    return s1 + s2;
}
...
test_struct foo(test_struct&& s)
{
    return s;
}

static void test_binder_move_constructor2()
{
    test_struct arg;
    auto src = n::bind(func_arb_arg<test_struct&>, arg);
    src();
    ...
    cpied = 0;
    test_struct arg1;
    auto f = bind(foo, _1)(std::move(arg1));
    (void)f;
